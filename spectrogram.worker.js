...
// Worker que processa o Float32Array recebido e devolve pixels RGBA do espectrograma (mel).
// Usa vendor/fft.js se disponível (importScripts).
// Mensagens:
//  - receber: {cmd:'process', audioBuffer: ArrayBuffer, sampleRate, options}
//  - postar: {type:'progress', value:0..1}
//  - postar: {type:'done', width, height, pixels: ArrayBuffer}

try {
  // tentar carregar FFT otimizada (local). Em file:// pode falhar em alguns navegadores.
  importScripts && importScripts('vendor/fft.js');
} catch (err) {
  // ignore; se não conseguir importar, podemos usar fallback (mas vendor/fft.js foi projetado para estar presente)
  // console.warn('importScripts vendor/fft.js falhou:', err);
}
...
(function() {
  ...
  // reuse FFT instances per size if FFT class is available
  ...
  self.addEventListener('message', function(e) {
    ...
    if (typeof self.FFT === 'function') {
      ...
      if (!fftInstance) {
        fftInstance = new self.FFT(fftSize);
        fftCache.set(fftSize, fftInstance);
      }
    }
    ...
    for (let f = 0; f < frames; f++) {
      ...
      for (let i = 0; i < fftSize; i++) {
        re[i] = (samples[offset + i] || 0) * windowFunc[i];
        im[i] = 0;
      }

      // FFT: use FFT instance if present, fallback to simple iterative one if needed
      if (fftInstance) {
        fftInstance.transform(re, im);
      } else {
        // minimal fallback iterative FFT (may be slower); tries to avoid throwing
        try {
          // naive DFT fallback (very slow) - but kept to avoid crash
      ...
      if ((f % progressStep) === 0) {
        self.postMessage({type:'progress', value: f / frames});
      }
    }
    ...
    if (logScale) {
      const top = maxVal;
      const bottom = Math.max(maxVal - dynamicRange, minVal);
    ...
    // retornar pixels como ArrayBuffer transferível
