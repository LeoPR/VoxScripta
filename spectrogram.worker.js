...
// Alterações:
// - Mantida a mesma escala/dimensões finais (com os mesmos limites mínimos/máximos).
// - Interpolação linear horizontal (entre frames) ao rasterizar, reduzindo “blocagem” sem mudar tamanho.
// - Telemetria leve: logs de tempos no console do worker e envio no payload 'done' (não obrigatório na UI).
...
(function(){
  ...
  function createMelFilterbank(nMels,fftSize,sampleRate,fmin,fmax){
    ...
    const fb=[];for(let m=0;m<nMels;m++){
      const lower=meltabs[m];const center=meltabs[m+1];const upper=meltabs[m+2];
      const filter=new Float32Array(binFreqs.length);
      const leftDen=(center-lower)||1e-9;const rightDen=(upper-center)||1e-9;
  ...
  function applyMelFilterbank(magSpectrum,melFilters){
    const nMels=melFilters.length;const out=new Float32Array(nMels);
    for(let m=0;m<nMels;m++){
      let sum=0;const filter=melFilters[m];
      for(let k=0;k<filter.length;k++){
        const f=filter[k];if(f){const v=magSpectrum[k];if(isFinite(v))sum+=v*f;}
      }
      out[m]=sum;
    }
    return out;
  ...
  function colorMap(v){
    if(!isFinite(v))v=0;if(v<0)v=0;if(v>1)v=1;
    const stops=[[68,1,84],[59,82,139],[33,144,140],[94,201,98],[253,231,37]];
    const t=v*(stops.length-1);const i=Math.floor(t);const frac=t-i;
  ...
  self.addEventListener('message',function(e){
    ...
    // Dimensões compatíveis (mantém mesmos limites de antes)
    ...
    // Preparar FFT (cache por tamanho)
    ...
    for(let f=0;f<frames;f++){
      ...
      if(fftInstance){fftInstance.transform(re,im);}
      else{
        try{
          ...
          for(let k=0;k<fftSize;k++){
            ...
            for(let n=0;n<fftSize;n++){
              ...
              sumRe+=re[n]*cos-im[n]*sin;
              sumIm+=re[n]*sin+im[n]*cos;
            }
      ...
      for(let k=0;k<half;k++){
        ...
        const mVal=Math.sqrt((isFinite(rr)?rr*rr:0)+(isFinite(ii)?ii*ii:0));
        mag[k]=isFinite(mVal)?mVal:0;
      }
    ...
    if(logScale){
      const top=maxVal;const bottom=Math.max(maxVal-dynamicRange,minVal);
    ...
    // Raster com interpolação linear horizontal entre frames (mesma escala final)
    // Mapeia x in [0..imgW-1] para fx in [0..frames-1], faz lerp entre floor(fx) e ceil(fx)
    for(let x=0;x<imgW;x++){
      const fx = (imgW>1) ? (x*(frames-1)/(imgW-1)) : 0;
      const fi = Math.floor(fx);
      const ci = Math.min(frames-1, fi+1);
      const frac = fx - fi;

      const baseF = fi * nMels;
      const baseC = ci * nMels;

      for(let y=0;y<imgH;y++){
        const melIdx=Math.floor((1-y/imgH)*(nMels-1));
        const vF = melDb[baseF + melIdx] || 0;
        const vC = melDb[baseC + melIdx] || 0;
        const v = vF + (vC - vF) * frac;
      ...
      if((x%Math.max(1,Math.floor(imgW/30)))===0){
        self.postMessage({type:'progress',value:0.95+0.05*(x/imgW)});
      }
    }
    const t4=performance.now();

    const timings = {
      stft_mel_ms: +(t2 - t1).toFixed(2),
      normalize_ms: +(t3 - t2).toFixed(2),
      raster_ms: +(t4 - t3).toFixed(2),
      total_ms: +(t4 - t0).toFixed(2)
    };
    // Telemetria no console do worker (aparece no DevTools)
    try { console.log('[spectrogram.worker] timings:', timings); } catch(_) {}
