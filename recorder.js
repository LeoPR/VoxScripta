// recorder.js — versão com applyAGC unificada (usa window.applyAGC de audio.js)
...
async function processAndPlayBlobDelegator(blob) {
  if (typeof window.processAndPlayBlob === 'function') {
    ...
    try {
      ...
      // Use applyAGC from audio.js
      const agcFunc = (window && window.applyAGC) ? window.applyAGC : null;
      let processed;
      let gain = 1;
      if (typeof agcFunc === 'function') {
        const agcRes = agcFunc(raw, (window.processingOptions && window.processingOptions.agc && window.processingOptions.agc.targetRMS) || 0.08, (window.processingOptions && window.processingOptions.agc && window.processingOptions.agc.maxGain) || 8, (window.processingOptions && window.processingOptions.agc && window.processingOptions.agc.limiterThreshold) || 0.99);
        processed = agcRes && agcRes.processed ? agcRes.processed : raw;
        gain = agcRes && agcRes.gain ? agcRes.gain : 1;
      } else {
        // fallback simple: no AGC available
        processed = raw;
        gain = 1;
      }
...
// ------------------------------
// (função applyAGC local removida — usa window.applyAGC de audio.js)
// ------------------------------
...
// ------------------------------
// Init: register worker.onmessage handler (if available) and load sessions (não bloqueante)
// ------------------------------
(function init() {
  try {
