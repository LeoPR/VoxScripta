...
async function processAndPlayBlobDelegator(blob) {
  if (typeof window.processAndPlayBlob === 'function') {
    ...
    try {
      ...
      const { processed } = window.applyAGC(
        raw,
        (window.processingOptions && window.processingOptions.agc && window.processingOptions.agc.targetRMS) || 0.08,
        (window.processingOptions && window.processingOptions.agc && window.processingOptions.agc.maxGain) || 8,
        (window.processingOptions && window.processingOptions.agc && window.processingOptions.agc.limiterThreshold) || 0.99
      );
