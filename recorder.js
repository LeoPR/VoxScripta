// recorder.js — HiDPI, canvas sizing ajustado, sem erro de sintaxe no worker inline
...
// Worker: inline fallback (corrigido, sem erro de sintaxe)
...
function drawWaveformPixels(width, height, pixels) {
  const dpr = window.devicePixelRatio || 1;
...
// processa blob: buffer transferido sem cópia
...
// desenho waveform ao vivo
function drawLiveWaveform() {
  if (!analyser) return;
  const bufferLength = analyser.fftSize;
  const dataArray = new Uint8Array(bufferLength);
  // Use o mesmo tamanho do canvas do spectrogram
  const dpr = window.devicePixelRatio || 1;
  const width = 940;
  const height = 180;
  waveform.width = width * dpr;
